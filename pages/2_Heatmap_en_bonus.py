...
# Create filters for department, month, and values to display
selected_department = st.selectbox('Select Department:', ['HR', 'Sales', 'IT'])
selected_month = st.selectbox('Select Month:', ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'])
display_absences = st.checkbox('Display Absences')
display_vacations = st.checkbox('Display Vacations')

# Filter the data based on user selections
filtered_data = employee_data[(employee_data['Department'] == selected_department) & (employee_data['Date'].dt.strftime('%B') == selected_month)]

# Select the values to display based on the checkboxes
if display_absences and display_vacations:
    values_column = 'Absences and Vacations'
    heatmap_data = filtered_data.pivot(index='Department', columns='Date', values=['Absences', 'Vacations'])
elif display_absences:
    values_column = 'Absences'
    heatmap_data = filtered_data.pivot(index='Department', columns='Date', values='Absences')
elif display_vacations:
    values_column = 'Vacations'
    heatmap_data = filtered_data.pivot(index='Department', columns='Date', values='Vacations')
else:
    st.warning('Please select at least one value to display.')
...
fig = px.imshow(
    ...
    title=f'Employee {values_column} Heatmap for {selected_department} in {selected_month}'
)
