...
for date in dates:
    for service in departments:
        ...
        vacances = np.random.randint(0, 3)  # Random daily vacations (0 to 2)
        data.append([date, service, absences, vacances])

# Create a DataFrame
employee_data = pd.DataFrame(data, columns=['Date', 'Service', 'Absences', 'Vacances'])
...
selected_department = st.selectbox('Select Service:', ['RH', 'Ventes', 'IT'])
...
filtered_data = employee_data[(employee_data['Service'] == selected_department) & (employee_data['Date'].dt.strftime('%B') == selected_month)]

# Pivot the filtered data for plotting
heatmap_data = filtered_data.pivot(index='Service', columns='Date', values='Absences')
...
fig = px.imshow(
    ...
    title=f'Carte thermique des absences des employés pour {selected_department} in {selected_month}'
)
